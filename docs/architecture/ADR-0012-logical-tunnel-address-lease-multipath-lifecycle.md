...
ADR-0014 is withdrawn/invalidated because it incorrectly globalized the per-lane single-flow invariant and falsely superseded these multipath/lifecycle clauses. ADR-0013 remains historical/withdrawn.
...
Policy targets are distinct from the architectural ceiling:
...
### 4. Game Lane semantics are the general multipath/replacement layer
...
### 5. Server assigns a unique tunnel address lease
...
### 6. Linux raw-IP data plane is shared TUN + one host NAT
...
### 7. Idle transport policy is based on real payload activity
...
### 8. Transport age rotation is independent of idle policy
...
### 9. Replacement is make-before-break and reuses race/dedup
...
### 11. Liveness and long-lived logical VPN behavior
...
### 12. Server restart is not transparent-mobility scope
...
### 13. Startup-latency optimization remains deferred
...
One public server `WBD_PORT` and one raw FakeTCP mux may accept many independent public 4-tuples, including multiple lanes belonging to one Logical Tunnel. Per-lane DTLS/LINK state feeds the Logical Tunnel Game/race aggregation. The final raw-IP path remains shared TUN + root routing + one WBD-owned host NAT.

One public server port does **not** mean one Transport Lane per Logical Tunnel.
...
- FEC release wire remains `off` or fixed systematic `20:20` unless a later explicitly authorized ADR reopens it;
- FEC state is lane-local and systematic sources are not intentionally delayed merely to fill a block;
...
- secrets must not be logged and per-packet INFO spam is forbidden;
...
6. candidate failure leaves the old healthy lane usable;
7. Game mode can replace one lane at a time, e.g. `A+B -> A+B+C -> B+C`;
8. distinct Logical Tunnels receive distinct leased IPs and remain isolated;
9. raw-IP source spoofing across leases is rejected;
10. DNS-style UDP, generic UDP and TCP pass shared TUN + one host NAT;
11. FEC `off` and `20:20` remain lane-local and qualified;
12. exact-head Windows and Linux builds/full-stack tests pass before physical Windows 11 -> Ubuntu ARM64 qualification.

## Supersession / invalidation notes
...
ADR-0014 is also withdrawn/invalidated. It incorrectly generalized this ADR's per-lane single-flow requirement into one public lane for the whole Logical Tunnel and falsely presented that change as a product-owner final freeze. No agent-authored ADR may infer a product-owner override without explicit live human authorization.
