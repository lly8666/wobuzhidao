...
`2026-09-06-lane-backend-refresh-stale-faketcp-runtime-log`

This generation starts a fresh exact-head hosted/package qualification after the physical 300-second development run exposed four independent runtime defects and one observability gap. The candidate carries the repaired lane lifecycle from the previous generation and adds: backend-pending application datagrams are non-fatal until a recognized authenticated service payload selects the backend; LINK refreshes authenticated Logical Tunnel metadata before long-idle Game payload so Game-side metadata expiry cannot orphan a still-live backend socket; both the initial authoritative Game lane and replacement candidates budget `gamelane.HeaderSize` above the configured inner/Wintun MTU; FakeTCP mux data associations are reclaimed after bounded client-RX silence before minute-scale ghost retransmissions continue; and the Windows GUI owns the runtime log handle and emits `WBD_RUNTIME_LOG_READY` so long physical tests retain child/runtime diagnostics. Rotation failure/retry and success paths also emit explicit policy/retry markers so a failed replacement retry cannot be confused with the configured lane-age interval.

The user-visible MTU remains the inner/Wintun MTU. The current default is 1360 and custom values remain accepted by the existing validation range; Game LINK plaintext budget is derived as inner MTU plus the fixed 32-byte Game envelope. Functional release qualification remains FEC off. Unknown or malformed payload remains rejected; the backend-pending change prevents premature session teardown but does not classify unknown data as Game or weaken authenticated Logical Tunnel metadata checks.

The immediately preceding `7b65cf93c4c72275957e0dc0906871fad83549b4` push gates were green, but its final test-only commit did not itself produce a same-SHA Windows portable artifact because that packaging workflow is path-filtered. Therefore those push results are development evidence only and are not package/release authority. This kick intentionally creates a new immutable candidate SHA, forces the release aggregator to dispatch the Windows portable and Linux release producers for that exact SHA, and requires all three artifact source fences plus all 31 hosted gates before any physical package is selected. No hosted/package or physical evidence from `7b65cf93c4c72275957e0dc0906871fad83549b4`, `37aea631797912f627dd36b723d0c509e5524163`, or any earlier SHA transfers to the fresh candidate produced by this generation.
