...
static double q(double *x, size_t n, double p) { if(!n) return NAN; qsort(x,n,sizeof(*x),cmpd); size_t i=(size_t)ceil(p*n)-1; if(i>=n)i=n-1; return x[i]; }

static int server(int port, double seconds, int drain_ms, const char *out) {
    int s=socket(AF_INET,SOCK_DGRAM,0); if(s<0){perror("socket");return 2;}
    ...
    for(;;){
        ...
        if(n>=24){
            ...
            if(tot==total && seq<total){
                if(seen[seq]) dup++;
                else { seen[seq]=1; lat[received++]=(double)(now-ts)/1e6; if(have&&seq<last)ooo++; last=seq; have=1; }
            }
        } else if(n<0 && errno!=EAGAIN && errno!=EWOULDBLOCK && errno!=EINTR){perror("recv");return 2;}
...
static int client(int port, double mbps, double seconds, int size) {
    if(size<24)size=24; uint64_t total=(uint64_t)floor(mbps*1000000.0*seconds/(size*8.0)); if(total<1)total=1;
    int s=socket(AF_INET,SOCK_DGRAM,0); if(s<0){perror("socket");return 2;} int sz=4<<20; setsockopt(s,SOL_SOCKET,SO_SNDBUF,&sz,sizeof(sz));
    struct sockaddr_in a={.sin_family=AF_INET,.sin_port=htons(port),.sin_addr.s_addr=htonl(INADDR_LOOPBACK)}; if(connect(s,(void*)&a,sizeof(a))<0){perror("connect");return 2;}
    uint8_t *buf=calloc(1,size); double pps=mbps*1000000.0/(size*8.0),step=1e9/pps; uint64_t t0=nowns(),sent=0;
    for(uint64_t seq=0;seq<total;seq++){
        uint64_t target=t0+(uint64_t)(seq*step); for(;;){uint64_t now=nowns(); if(now>=target)break; uint64_t d=target-now; if(d>50000){struct timespec t={0,(long)(d-20000)};nanosleep(&t,NULL);} else __asm__ __volatile__("pause");}
        uint64_t ts=nowns(); memcpy(buf,&seq,8); memcpy(buf+8,&total,8); memcpy(buf+16,&ts,8); if(send(s,buf,size,0)==size)sent++;
    }
    fprintf(stdout,"ONEWAY_SENT planned=%"PRIu64" sent=%"PRIu64" active_s=%.6f\n",total,sent,(double)(nowns()-t0)/1e9);
    free(buf);close(s);return 0;
}

int main(int argc,char **argv){
    if(argc==6 && !strcmp(argv[1],"server")) return server(atoi(argv[2]),atof(argv[3]),atoi(argv[4]),argv[5]);
    if(argc==6 && !strcmp(argv[1],"client")) return client(atoi(argv[2]),atof(argv[3]),atof(argv[4]),atoi(argv[5]));
