...
CANDIDATE_LPORT_BASE=$(( CANDIDATE_FPORT_BASE - CANDIDATE_PORT_SPAN - 64 ))
(( CANDIDATE_LPORT_BASE > 1024 )) || {
  echo "ephemeral range leaves no safe candidate harness ports: low=$EPHEMERAL_PORT_LOW rotations=$ROTATIONS" >&2
  exit 2
}
echo "WBD_SOAK_CANDIDATE_PORT_PLAN ephemeral=$EPHEMERAL_PORT_LOW-$EPHEMERAL_PORT_HIGH faketcp_base=$CANDIDATE_FPORT_BASE dtls_base=$CANDIDATE_DPORT_BASE link_base=$CANDIDATE_LPORT_BASE max_generation=$MAX_LANE_GEN"
