...
func parseTunnelRoutes4(raw string) ([]netip.Prefix, error) {
	...
	for _, part := range parts {
		...
		if err != nil || !p.Addr().Is4() {
			return nil, fmt.Errorf("invalid tunnel IPv4 route %q", part)
		}
		routes = append(routes, p.Masked())
	}
	if len(routes) == 0 {
		return nil, errors.New("at least one tunnel IPv4 route is required")
	}
	return routes, nil
}
...
func newMuxServer(c config) (*muxServer, error) {
	...
	if c.bootstrapEnabled() {
		...
		if _, err := net.ResolveTCPAddr("tcp", c.fallbackTarget); err != nil {
			return nil, errors.New("invalid --fallback-target")
		}
		pool, err := netip.ParsePrefix(strings.TrimSpace(c.tunnelPool))
		if err != nil {
			return nil, errors.New("invalid --tunnel-pool")
		}
		routes, err := parseTunnelRoutes4(c.tunnelRoutes4)
		if err != nil {
			return nil, err
		}
		tunnelManager, err = logicaltunnel.NewManager(pool, routes)
		if err != nil {
			return nil, err
		}
	}
...
func (s *muxServer) rawLoop() error {
	...
	for {
		...
		if s.handlePeerReset(flow, sess, seg) {
			continue
		}
		now := time.Now()
		...
		if sess.assoc.State() == faketcp.ServerAssociationAwaitACK {
			...
			if s.cfg.bootstrapEnabled() {
				if err := s.startBootstrap(sess); err != nil {
		...
		res, err := sess.assoc.HandleSegment(seg, now)
		if err != nil {
			continue
		}
...
func (s *muxServer) handlePeerReset(flow faketcp.ServerFlow, sess *muxSession, seg faketcp.Segment) bool {
	if seg.Flags&faketcp.FlagRST == 0 {
		return false
	}
	fmt.Printf("WBD_FAKETCP_MUX_PEER_RESET client=%d server=%d\n", flow.ClientPort, flow.ServerPort)
	s.removeSessionMatch(flow, sess)
	return true
}
...
func (s *muxServer) acceptSYN(seg faketcp.Segment) error {
	...
	go func() {
		t := time.NewTimer(halfOpenTimeout)
		defer t.Stop()
		select {
		case <-s.ctx.Done():
			return
...
func (s *muxServer) startBootstrap(sess *muxSession) error {
	...
	sess.mu.Lock()
	sess.stage = stageBootstrap
	sess.bootstrap = stream
	sess.mu.Unlock()
...
func (s *muxServer) runBootstrap(sess *muxSession, stream *faketcp.BootstrapStream) {
	...
	auth, err := realityfront.BootstrapServerRecognizedSingleFlowV2(s.ctx, stream, hello.Raw, realityfront.SingleFlowServerV2Config{
		SingleFlowServerConfig: realityfront.SingleFlowServerConfig{
			...
			RouteKey:         []byte(s.cfg.routeKey),
			ExpectedUsername: s.cfg.username,
			ExpectedPassword: s.cfg.password,
			TicketDir:        s.cfg.ticketDir,
			TLSConfig:        s.frontTLS,
			Timeout:          s.cfg.bootstrapTimeout,
		},
...
func (s *muxServer) runFallback(sess *muxSession, stream *faketcp.BootstrapStream, rawHello []byte) {
	...
	if err := writeFull(upstream, rawHello); err != nil {
		s.removeSessionMatch(sess.flow, sess)
		return
	}
...
func (s *muxServer) waitSessionAck(sess *muxSession, end uint32, deadline time.Time) error {
	for {
		if sess.assoc.SenderLastAck() == end {
			return nil
		}
...
func (s *muxServer) activateDTLS(sess *muxSession) error {
	...
	if sess.worker != nil || sess.relay != nil {
		...
		_ = relay.Close()
		_ = worker.Stop()
		return errors.New("DTLS worker already active")
	}
