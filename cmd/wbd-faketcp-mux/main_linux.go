...
func main() {
	...
	fmt.Printf("READY role=server-mux listen=%s max_sessions=%d recovery=%s link_target=%s single_flow_bootstrap=%t\n", c.listen, c.maxSessions, c.recovery, c.linkTarget, c.bootstrapEnabled())
...
func (c config) bootstrapEnabled() bool {
	return strings.TrimSpace(c.serverName) != "" || strings.TrimSpace(c.routeKey) != "" || strings.TrimSpace(c.username) != "" ||
		strings.TrimSpace(c.password) != "" || strings.TrimSpace(c.ticketDir) != "" || strings.TrimSpace(c.frontCert) != "" || strings.TrimSpace(c.frontKey) != ""
}
...
func usage() {
	...
	fmt.Fprintln(os.Stderr, "  product single-flow mode additionally requires --front-cert --front-key --server-name --route-key --username --password --ticket-dir")
}
...
func newMuxServer(c config) (*muxServer, error) {
	...
	if c.bootstrapEnabled() {
		if c.frontCert == "" || c.frontKey == "" || c.serverName == "" || len(c.routeKey) < 16 || c.username == "" || c.password == "" || c.ticketDir == "" || c.bootstrapTimeout <= 0 {
			return nil, errors.New("single-flow bootstrap requires front cert/key, server-name, route-key >=16 bytes, username/password, ticket-dir and positive timeout")
		}
	}
	...
	if err := syscall.SetsockoptInt(fd, syscall.IPPROTO_IP, syscall.IP_HDRINCL, 1); err != nil {
		_ = syscall.Close(fd); return nil, err
	}
	...
	if c.bootstrapEnabled() {
		...
		if err != nil { _ = syscall.Close(fd); return nil, err }
...
func (s *muxServer) rawLoop() error {
	...
	for {
		...
		if err != nil {
			select { case <-s.ctx.Done(): return s.ctx.Err(); default: return err }
		}
		seg, err := faketcp.ParseIPv4TCP(buf[:n])
		if err != nil || seg.DstIP != s.serverIP || seg.DstPort != s.serverPort { continue }
		...
		if sess.assoc.State() == faketcp.ServerAssociationAwaitACK {
			...
			if s.cfg.bootstrapEnabled() {
				if err := s.startBootstrap(sess); err != nil { s.removeSessionMatch(flow, sess); continue }
			} else {
				if err := s.activateDTLS(sess); err != nil { s.removeSessionMatch(flow, sess); continue }
			}
			// Do not continue: a TCP final ACK may carry the first TLS/DTLS bytes.
		}
		...
		if res.FastRetransmit != nil {
			if err := s.sendPending(sess, res.FastRetransmit); err != nil { return err }
		}
		if res.AckNeeded {
			if err := s.sendACK(sess, res.Ack, res.SACK[:res.SACKN]); err != nil { return err }
		}
		if len(res.Deliver) != 0 {
			if err := sess.routeDeliver(res.DeliverSeq, res.Deliver); err != nil {
...
func (s *muxServer) acceptSYN(seg faketcp.Segment) error {
	...
	if _, exists := s.sessions[flow]; exists {
		s.mu.Unlock(); s.table.Remove(flow); return faketcp.ErrAssociationExists
	}
	s.sessions[flow] = sess
	s.mu.Unlock()
	if err := s.sendSYNACK(sess); err != nil {
		s.removeSessionMatch(flow, sess); return err
	}
	go func() {
		...
		case <-t.C:
			if sess.assoc.State() == faketcp.ServerAssociationAwaitACK { s.removeSessionMatch(flow, sess) }
		}
	}()
	return nil
}
...
func (s *muxServer) startBootstrap(sess *muxSession) error {
	...
	stream, err := faketcp.NewBootstrapStream(sess.assoc.ReceiverNext(), func(payload []byte) (uint32, error) {
		p, err := sess.assoc.Enqueue(payload, time.Now())
		if err != nil { return 0, err }
		if err := s.sendPending(sess, p); err != nil { return 0, err }
		return p.End, nil
	}, func(end uint32, deadline time.Time) error {
		return s.waitSessionAck(sess, end, deadline)
	}, local, remote)
	if err != nil { return err }
...
func (s *muxServer) runBootstrap(sess *muxSession, stream *faketcp.BootstrapStream) {
	_, err := realityfront.BootstrapServerSingleFlow(s.ctx, stream, realityfront.SingleFlowServerConfig{
	...
	// TLS/auth bytes are ACK-gated. Enter transition before DTLS worker startup so
	// any immediately arriving client DTLS datagrams are queued, never fed back
	// into the ordered TLS stream and never dropped during worker creation.
	sess.mu.Lock()
	if sess.bootstrap == stream {
		sess.bootstrap = nil
...
func (s *muxServer) waitSessionAck(sess *muxSession, end uint32, deadline time.Time) error {
	for {
		...
		if !deadline.IsZero() {
			...
			t = time.NewTimer(d); timer = t.C
		}
		select {
		case <-sess.ackNotify:
		case <-s.ctx.Done(): if t != nil { t.Stop() }; return s.ctx.Err()
		case <-timer: return faketcp.ErrBootstrapTimeout
		}
		if t != nil { t.Stop() }
	}
}

func (sess *muxSession) signalAck() {
	select { case sess.ackNotify <- struct{}{}: default: }
}

func (sess *muxSession) routeDeliver(seq uint32, payload []byte) error {
	sess.mu.Lock()
	if sess.bootstrap != nil {
		stream := sess.bootstrap
		sess.mu.Unlock()
	...
	if sess.stage == stageTransition {
		if len(sess.pendingData) >= 64 {
			sess.mu.Unlock()
			return errors.New("single-flow transition queue full")
		}
		sess.pendingData = append(sess.pendingData, append([]byte(nil), payload...))
		sess.mu.Unlock()
		return nil
	}
	sess.mu.Unlock()
...
func (s *muxServer) activateDTLS(sess *muxSession) error {
	...
	if err != nil { _ = worker.Stop(); return err }

	sess.mu.Lock()
	if sess.worker != nil || sess.relay != nil {
		sess.mu.Unlock(); _ = relay.Close(); _ = worker.Stop(); return errors.New("DTLS worker already active")
	}
	pending := sess.pendingData
	...
	for _, p := range pending {
		if _, err := relay.Write(p); err != nil { return err }
	}
	go s.relayLoop(sess)
...
func (s *muxServer) relayLoop(sess *muxSession) {
	buf := make([]byte, 65535)
	for {
		sess.mu.RLock(); relay := sess.relay; sess.mu.RUnlock()
		if relay == nil { return }
...
func (s *muxServer) sendRaw(flow faketcp.ServerFlow, seq, ack uint32, flags uint8, sacks []faketcp.SACKBlock, payload []byte) error {
	s.sendMu.Lock(); defer s.sendMu.Unlock()
...
func (s *muxServer) getSession(flow faketcp.ServerFlow) *muxSession {
	s.mu.RLock(); sess := s.sessions[flow]; s.mu.RUnlock(); return sess
}

func (s *muxServer) snapshotSessions() []*muxSession {
	s.mu.RLock(); out := make([]*muxSession, 0, len(s.sessions)); for _, sess := range s.sessions { out = append(out, sess) }; s.mu.RUnlock(); return out
}

func (s *muxServer) removeSessionMatch(flow faketcp.ServerFlow, expected *muxSession) {
	s.mu.Lock()
	sess := s.sessions[flow]
	if sess != expected { s.mu.Unlock(); return }
	delete(s.sessions, flow)
	s.mu.Unlock()
	if sess == nil { return }
	sess.mu.Lock()
	stream := sess.bootstrap; sess.bootstrap = nil
	relay := sess.relay; sess.relay = nil
	worker := sess.worker; sess.worker = nil
	sess.pendingData = nil
	sess.mu.Unlock()
	if stream != nil { _ = stream.Close() }
	if relay != nil { _ = relay.Close() }
	if worker != nil { _ = worker.Stop() }
	s.table.Remove(flow)
}

func (s *muxServer) Close() {
	if s.cancel != nil { s.cancel() }
	for _, sess := range s.snapshotSessions() { s.removeSessionMatch(sess.flow, sess) }
	if s.fd >= 0 { _ = syscall.Close(s.fd); s.fd = -1 }
}
