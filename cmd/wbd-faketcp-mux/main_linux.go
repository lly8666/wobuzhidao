...
func (s *muxServer) rawLoop() error {
	...
	for {
		...
		// Allocate wolfSSL only when this established flow actually carries its
		// first payload. The triggering segment is retained in userspace until the
		// worker is ready, so the first DTLS datagram is not sacrificed to startup.
		if sess.worker == nil && len(seg.Payload) != 0 {
...
func (s *muxServer) acceptSYN(seg faketcp.Segment) error {
	...
	// Half-open raw associations own only transport state. Delay the DTLS worker
	// until the first established payload actually needs it.
...
func (s *muxServer) startSessionWorker(sess *muxSession) error {
	...
	sess.relay = relay
	go s.relayLoop(sess)
	go func(flow faketcp.ServerFlow, w *dtlsworker.Worker) {
		_ = w.Wait()
		s.removeSession(flow)
	}(sess.flow, worker)
	return nil
