...
// windowsHandshakeCapture models the boundary that matters on a physical
// Windows adapter: Npcap sees unrelated traffic, but only the exact inbound WBD
// four-tuple is allowed to reach the shared FakeTCP handshake state machine.
...
func (r *windowsHandshakeCapture) WritePacket(packet []byte, _ [4]byte) error {
	...
	// sees after the adapter-level filtering above.
...
func TestWindowsAdapterNoiseStillCompletesFakeTCPHandshake(t *testing.T) {
	...
	if ack.Flags != faketcp.FlagACK || ack.Seq != syn.Seq+1 || ack.Ack != uint32(0x4a17c003)+1 {
		t.Fatalf("final ACK mismatch: flags=%02x seq=%d want=%d ack=%d want=%d",
			ack.Flags, ack.Seq, syn.Seq+1, ack.Ack, uint32(0x4a17c003)+1)
	}
