...
func run() error {
	...
	wc := wndClassEx{CbSize: uint32(unsafe.Sizeof(wndClassEx{})), Style: csHRedraw | csVRedraw, WndProc: syscall.NewCallback(windowProc), Instance: instance, Icon: icon, Cursor: cursor, Background: colorWindow + 1, ClassName: className, IconSm: icon}
...
func windowProc(hwnd uintptr, message uint32, wParam, lParam uintptr) uintptr {
	...
	case wmCommand:
		...
		case idDisconnectButton:
			...
			return 0
		case idReconnectButton:
			beginReconnect(hwnd)
			return 0
		case idNpcapButton:
			beginNpcapSetup(hwnd)
			return 0
		case idHideButton:
			minimizeToTray(hwnd)
			return 0
...
func createControls(hwnd uintptr) {
	...
	app.proxyLAN = createCheckbox(hwnd, "Proxy LAN/private IPv4 (RFC1918)", 24, 108, 225, 28, idProxyLAN)
	setChecked(app.proxyLAN, policy.ProxyLAN)
	app.proxyChina = createCheckbox(hwnd, "Proxy mainland China IPv4", 258, 108, 205, 28, idProxyChina)
	setChecked(app.proxyChina, policy.ProxyChina)
	app.proxyOther = createCheckbox(hwnd, "Proxy all other IPv4", 478, 108, 190, 28, idProxyOther)
	setChecked(app.proxyOther, policy.ProxyOther)
...
func createControl(parent uintptr, class, text string, x, y, width, height int, id uintptr) uintptr {
	...
	hwnd, _, _ := procCreateWindowExW.Call(0, uintptr(unsafe.Pointer(utf16Ptr(class))), uintptr(unsafe.Pointer(utf16Ptr(text))), wsChild|wsVisible|bsPushButton, uintptr(x), uintptr(y), uintptr(width), uintptr(height), parent, id, instance, 0)
	if hwnd != 0 && app.font != 0 {
		procSendMessageW.Call(hwnd, wmSetFont, app.font, 1)
	}
	return hwnd
}
func createCheckbox(parent uintptr, text string, x, y, width, height int, id uintptr) uintptr {
	instance, _, _ := procGetModuleHandleW.Call(0)
	hwnd, _, _ := procCreateWindowExW.Call(0, uintptr(unsafe.Pointer(utf16Ptr("BUTTON"))), uintptr(unsafe.Pointer(utf16Ptr(text))), wsChild|wsVisible|bsAutoCheckbox, uintptr(x), uintptr(y), uintptr(width), uintptr(height), parent, id, instance, 0)
	if hwnd != 0 && app.font != 0 {
		procSendMessageW.Call(hwnd, wmSetFont, app.font, 1)
	}
	return hwnd
...
func isChecked(hwnd uintptr) bool {
	...
	v, _, _ := procSendMessageW.Call(hwnd, bmGetCheck, 0, 0)
	return v == bstChecked
}
...
func launchDisconnect() {
	...
	if app.exitRequested {
		setStatus("Status: exiting; removing routes, DNS and IPv6 block before reverse teardown")
...
func beginReconnect(hwnd uintptr) {
	if app.operation != "" || app.exitRequested || app.controller.State() != windowsruntime.RuntimeConnected {
		return
	}
...
func beginNpcapSetup(hwnd uintptr) {
	if app.operation != "" || app.exitRequested || app.controller.State() != windowsruntime.RuntimeDisconnected {
		return
	}
	exe, err := os.Executable()
	if err != nil {
		messageBox("WBD Npcap", err.Error())
		return
	}
	...
	go func() {
		...
		if runErr != nil {
			runErr = fmt.Errorf("Npcap setup: %w: %s", runErr, detail)
		}
		postRuntimeResult(runtimeResult{action: "npcap", err: runErr, detail: detail})
	}()
}
...
func requestExit(hwnd uintptr) {
	...
	if app.operation == "npcap" {
		setStatus("Status: Exit requested; waiting for Npcap setup to finish")
		refreshControls()
		return
	}
	if app.operation == "connect" {
		setStatus("Status: Exit requested; waiting for connection setup before cleanup")
		refreshControls()
		return
	}
	if app.operation == "reconnect" {
		setStatus("Status: Exit requested; waiting for transport rotation before cleanup")
		refreshControls()
		return
	}
	if app.operation == "disconnect" {
		setStatus("Status: Exit requested; waiting for network cleanup and teardown")
		refreshControls()
		return
	}
	launchDisconnect()
...
func handleRuntimeResult(hwnd uintptr) {
	...
	case "reconnect":
		if result.err != nil {
			setStatus("Status: connected; transport reconnect failed: " + result.err.Error())
			if !app.exitRequested {
	...
	case "disconnect":
		if result.err != nil {
			...
			setStatus("Status: disconnected, but network cleanup needs retry: " + result.err.Error())
...
func refreshControls() {
	...
	disconnected := state == windowsruntime.RuntimeDisconnected
	connected := state == windowsruntime.RuntimeConnected
	connectEnabled := app.profileReady && !busy && !app.exitRequested && !app.cleanupFailed && disconnected
	disconnectEnabled := !busy && !app.exitRequested && (app.cleanupFailed || connected)
	reconnectEnabled := !busy && !app.exitRequested && !app.cleanupFailed && connected
	npcapEnabled := !busy && !app.exitRequested && !app.cleanupFailed && disconnected
	routingEnabled := !busy && !app.exitRequested && !app.cleanupFailed && disconnected
	setEnabled(app.connectButton, connectEnabled)
	setEnabled(app.disconnectButton, disconnectEnabled)
	setEnabled(app.reconnectButton, reconnectEnabled)
	setEnabled(app.npcapButton, npcapEnabled)
...
func deleteTrayIcon(hwnd uintptr) {
	...
	nid := notifyIconData{CbSize: uint32(unsafe.Sizeof(notifyIconData{})), HWnd: hwnd, UID: trayIconID}
...
func messageBoxInfo(title, text string) {
	proc := user32.NewProc("MessageBoxW")
	proc.Call(0, uintptr(unsafe.Pointer(utf16Ptr(text))), uintptr(unsafe.Pointer(utf16Ptr(title))), 0x40)
}
