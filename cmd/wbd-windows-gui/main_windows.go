...
const (
	csHRedraw=0x0002; csVRedraw=0x0001
	...
	sizeMinimized=1; swHide=0; swNormal=1; swRestore=9
...
var (
	user32=syscall.NewLazyDLL("user32.dll"); shell32=syscall.NewLazyDLL("shell32.dll"); kernel32=syscall.NewLazyDLL("kernel32.dll"); gdi32=syscall.NewLazyDLL("gdi32.dll")
	...
	procShowWindow=user32.NewProc("ShowWindow"); procUpdateWindow=user32.NewProc("UpdateWindow"); procGetMessageW=user32.NewProc("GetMessageW"); procTranslateMessage=user32.NewProc("TranslateMessage"); procDispatchMessageW=user32.NewProc("DispatchMessageW")
	procPostQuitMessage=user32.NewProc("PostQuitMessage"); procPostMessageW=user32.NewProc("PostMessageW"); procLoadIconW=user32.NewProc("LoadIconW"); procLoadCursorW=user32.NewProc("LoadCursorW"); procSendMessageW=user32.NewProc("SendMessageW")
	procSetWindowTextW=user32.NewProc("SetWindowTextW"); procEnableWindow=user32.NewProc("EnableWindow"); procSetForegroundWindow=user32.NewProc("SetForegroundWindow"); procCreatePopupMenu=user32.NewProc("CreatePopupMenu")
	procAppendMenuW=user32.NewProc("AppendMenuW"); procTrackPopupMenu=user32.NewProc("TrackPopupMenu"); procDestroyMenu=user32.NewProc("DestroyMenu"); procGetCursorPos=user32.NewProc("GetCursorPos"); procRegisterWindowMessageW=user32.NewProc("RegisterWindowMessageW")
	procShellNotifyIconW=shell32.NewProc("Shell_NotifyIconW"); procGetModuleHandleW=kernel32.NewProc("GetModuleHandleW"); procGetStockObject=gdi32.NewProc("GetStockObject")
)

type point struct{X,Y int32}
type msg struct{HWnd uintptr; Message uint32; _ uint32; WParam,LParam uintptr; Time uint32; Pt point; LPrivate uint32}
type wndClassEx struct{CbSize,Style uint32; WndProc uintptr; ClsExtra,WndExtra int32; Instance,Icon,Cursor,Background uintptr; MenuName,ClassName *uint16; IconSm uintptr}
type notifyIconData struct{CbSize uint32; HWnd uintptr; UID,UFlags,UCallbackMessage uint32; HIcon uintptr; SzTip [128]uint16; DwState,DwStateMask uint32; SzInfo [256]uint16; UTimeoutOrVersion uint32; SzInfoTitle [64]uint16; DwInfoFlags uint32; GuidItem [16]byte; HBalloonIcon uintptr}
...
var app struct{
	...
	operation string
	exitRequested,cleanupFailed bool
}
...
func main(){
	...
	flag.Parse()
	app.state=windowsgui.NewWindowState(*startMinimized); app.controller=windowsruntime.NewController(nil,nil,nil); app.results=make(chan runtimeResult,4); app.profilePath=*profilePath
	if *profilePath!="" { if err:=loadRuntimeProfile(*profilePath); err!=nil { app.profileErr=err; messageBox("WBD Windows GUI profile",err.Error()) } }
	runtime.LockOSThread(); exitCode:=0
	if err:=run(); err!=nil { messageBox("WBD Windows GUI",err.Error()); exitCode=1 }
	if err:=cleanupBeforeProcessExit(); err!=nil { messageBox("WBD cleanup before exit",err.Error()); exitCode=1 }
	runtime.UnlockOSThread(); if exitCode!=0 { os.Exit(exitCode) }
}

func cleanupBeforeProcessExit() error { if app.controller==nil{return nil}; if app.operation!="" { <-app.results; app.operation="" }; return app.controller.Disconnect() }
func loadRuntimeProfile(path string) error {
	exe,err:=os.Executable(); if err!=nil{return fmt.Errorf("resolve GUI executable: %w",err)}
	programData:=os.Getenv("ProgramData"); if programData==""{return fmt.Errorf("ProgramData is not set")}
	stateDir:=filepath.Join(programData,"WBD"); if err:=os.MkdirAll(stateDir,0o700);err!=nil{return fmt.Errorf("create WBD state directory: %w",err)}
	profile,err:=windowsgui.LoadRuntimeProfile(path,filepath.Dir(exe),stateDir); if err!=nil{return err}; app.profile=profile;app.profileReady=true;return nil
}

func run() error {
	instance,_,err:=procGetModuleHandleW.Call(0);if instance==0{return fmt.Errorf("GetModuleHandleW: %v",err)}
	icon,_,_:=procLoadIconW.Call(0,idiApplication);cursor,_,_:=procLoadCursorW.Call(0,idcArrow);font,_,_:=procGetStockObject.Call(defaultGUIFont);app.icon=icon;app.font=font
	className:=utf16Ptr("WBDWindowsGUIWindow"); wc:=wndClassEx{CbSize:uint32(unsafe.Sizeof(wndClassEx{})),Style:csHRedraw|csVRedraw,WndProc:syscall.NewCallback(windowProc),Instance:instance,Icon:icon,Cursor:cursor,Background:colorWindow+1,ClassName:className,IconSm:icon}
	if r,_,callErr:=procRegisterClassExW.Call(uintptr(unsafe.Pointer(&wc)));r==0{return fmt.Errorf("RegisterClassExW: %v",callErr)}
	app.taskbarCreated=registerWindowMessage("TaskbarCreated");title:=utf16Ptr("WBD Windows Client")
	hwnd,_,callErr:=procCreateWindowExW.Call(0,uintptr(unsafe.Pointer(className)),uintptr(unsafe.Pointer(title)),wsOverlappedWindow,180,140,780,350,0,0,instance,0);if hwnd==0{return fmt.Errorf("CreateWindowExW: %v",callErr)};app.window=hwnd
	if app.state.Visible{procShowWindow.Call(hwnd,swNormal);procUpdateWindow.Call(hwnd)}else{procShowWindow.Call(hwnd,swHide)}
	var m msg;for{r,_,callErr:=procGetMessageW.Call(uintptr(unsafe.Pointer(&m)),0,0,0);if int32(r)==-1{return fmt.Errorf("GetMessageW: %v",callErr)};if r==0{return nil};procTranslateMessage.Call(uintptr(unsafe.Pointer(&m)));procDispatchMessageW.Call(uintptr(unsafe.Pointer(&m)))}
}

func windowProc(hwnd uintptr,message uint32,wParam,lParam uintptr) uintptr{
	if app.taskbarCreated!=0&&message==app.taskbarCreated{_ = addTrayIcon(hwnd);return 0}
	switch message{
	...
	r,_,_:=procDefWindowProcW.Call(hwnd,uintptr(message),wParam,lParam);return r
}
...
func beginNpcapSetup(hwnd uintptr){
	...
	exe,err:=os.Executable();if err!=nil{messageBox("WBD Npcap",err.Error());return};script:=filepath.Join(filepath.Dir(exe),"windows_npcap_prepare.ps1")
	app.operation="npcap";setStatus("Status: Npcap setup; downloading and verifying official installer");refreshControls()
...
func restoreFromTray(hwnd uintptr){app.state.Restore();procShowWindow.Call(hwnd,swRestore);procSetForegroundWindow.Call(hwnd)}
func finalizeExit(hwnd uintptr){app.state.Exit();deleteTrayIcon(hwnd);procPostQuitMessage.Call(0)}

func addTrayIcon(hwnd uintptr)error{nid:=notifyIconData{CbSize:uint32(unsafe.Sizeof(notifyIconData{})),HWnd:hwnd,UID:trayIconID,UFlags:nifMessage|nifIcon|nifTip,UCallbackMessage:trayCallbackMessage,HIcon:app.icon};copyUTF16(nid.SzTip[:],"WBD Windows Client");r,_,callErr:=procShellNotifyIconW.Call(nimAdd,uintptr(unsafe.Pointer(&nid)));if r==0{return fmt.Errorf("Shell_NotifyIconW(NIM_ADD): %v",callErr)};return nil}
func deleteTrayIcon(hwnd uintptr){if hwnd==0{return};nid:=notifyIconData{CbSize:uint32(unsafe.Sizeof(notifyIconData{})),HWnd:hwnd,UID:trayIconID};procShellNotifyIconW.Call(nimDelete,uintptr(unsafe.Pointer(&nid)))}
func showTrayMenu(hwnd uintptr){menu,_,_:=procCreatePopupMenu.Call();if menu==0{return};defer procDestroyMenu.Call(menu);appendMenu(menu,mfString,idTrayShow,"Show WBD");appendMenu(menu,mfSeparator,0,"");appendMenu(menu,mfString,idTrayExit,"Exit WBD");var p point;procGetCursorPos.Call(uintptr(unsafe.Pointer(&p)));procSetForegroundWindow.Call(hwnd);procTrackPopupMenu.Call(menu,tpmRightButton|tpmBottomAlign,uintptr(p.X),uintptr(p.Y),0,hwnd,0)}
func appendMenu(menu uintptr,flags,id uintptr,text string){var ptr uintptr;if text!=""{ptr=uintptr(unsafe.Pointer(utf16Ptr(text)))};procAppendMenuW.Call(menu,flags,id,ptr)}
func registerWindowMessage(name string)uint32{r,_,_:=procRegisterWindowMessageW.Call(uintptr(unsafe.Pointer(utf16Ptr(name))));return uint32(r)}
func messageBox(title,text string){proc:=user32.NewProc("MessageBoxW");proc.Call(0,uintptr(unsafe.Pointer(utf16Ptr(text))),uintptr(unsafe.Pointer(utf16Ptr(title))),0x10)}
func messageBoxInfo(title,text string){proc:=user32.NewProc("MessageBoxW");proc.Call(0,uintptr(unsafe.Pointer(utf16Ptr(text))),uintptr(unsafe.Pointer(utf16Ptr(title))),0x40)}
func utf16Ptr(s string)*uint16{p,err:=syscall.UTF16PtrFromString(s);if err!=nil{panic(err)};return p}
func copyUTF16(dst []uint16,src string){encoded,err:=syscall.UTF16FromString(src);if err!=nil{return};if len(encoded)>len(dst){encoded=encoded[:len(dst)];encoded[len(encoded)-1]=0};copy(dst,encoded)}
