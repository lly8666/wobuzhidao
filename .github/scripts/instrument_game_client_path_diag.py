...
# The baseline returns immediately on app WriteToUDP failure.  Count successful
# WBD1 application deliveries immediately before the existing delivered counter;
# Game->LINK write failures are separately counted by diagLaneWriteErr.  This
# keeps the diagnostic patch off the baseline error-control-flow line.
old = '''\t\tatomic.AddUint64(&c.delivered, 1)'''
new = '''\t\tif len(result.Payload) >= 4 && string(result.Payload[:4]) == "WBD1" {\n\t\t\tatomic.AddUint64(&c.diagAppTXPackets, 1)\n\t\t\tatomic.AddUint64(&c.diagAppTXBytes, uint64(len(result.Payload)))\n\t\t}\n\t\tatomic.AddUint64(&c.delivered, 1)'''
if s.count(old) != 1:
    raise SystemExit(f"delivered marker drift: {s.count(old)}")
s = s.replace(old, new, 1)

insert_before = '''func (c *client) appLoop() error {'''
diag = '''func (c *client) diagPathLoop() {\n\tt := time.NewTicker(time.Second)\n\tdefer t.Stop()\n\tfor now := range t.C {\n\t\tfmt.Printf("WBD_GAME_PATH_DIAG epoch_ns=%d app_rx_packets=%d app_rx_bytes=%d lane_tx_bytes=%d lane_rx_bytes=%d lane_write_err=%d app_tx_packets=%d app_tx_bytes=%d\\n",\n\t\t\tnow.UnixNano(),\n\t\t\tatomic.LoadUint64(&c.diagAppRXPackets), atomic.LoadUint64(&c.diagAppRXBytes),\n\t\t\tatomic.LoadUint64(&c.diagLaneTXBytes), atomic.LoadUint64(&c.diagLaneRXBytes),\n\t\t\tatomic.LoadUint64(&c.diagLaneWriteErr), atomic.LoadUint64(&c.diagAppTXPackets),\n\t\t\tatomic.LoadUint64(&c.diagAppTXBytes))\n\t}\n}\n\n'''
...
new = '''\tfmt.Printf("WBD_GAME_LANE_CLIENT_STATS logical_tx=%d delivered=%d duplicate=%d stale=%d lane_fail=%d dormant_drop=%d\\n",\n\t\tatomic.LoadUint64(&c.logicalTX), atomic.LoadUint64(&c.delivered), atomic.LoadUint64(&c.duplicate), atomic.LoadUint64(&c.stale), atomic.LoadUint64(&c.laneFail), atomic.LoadUint64(&c.dormantDrop))\n\tfmt.Printf("WBD_GAME_PATH_FINAL app_rx_packets=%d app_rx_bytes=%d lane_tx_bytes=%d lane_rx_bytes=%d lane_write_err=%d app_tx_packets=%d app_tx_bytes=%d\\n",\n\t\tatomic.LoadUint64(&c.diagAppRXPackets), atomic.LoadUint64(&c.diagAppRXBytes),\n\t\tatomic.LoadUint64(&c.diagLaneTXBytes), atomic.LoadUint64(&c.diagLaneRXBytes),\n\t\tatomic.LoadUint64(&c.diagLaneWriteErr), atomic.LoadUint64(&c.diagAppTXPackets),\n\t\tatomic.LoadUint64(&c.diagAppTXBytes))'''
if s.count(old) != 1:
    raise SystemExit(f"final stats marker drift: {s.count(old)}")
s = s.replace(old, new, 1)

p.write_text(s)
print("WBD_GAME_CLIENT_PATH_DIAG_PATCHED interval_sec=1 per_packet_logging=0 final_exact=1 delivery_counter_anchor=1")
