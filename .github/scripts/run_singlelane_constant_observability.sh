...
: "${LOSS_PCT:?loss pct required}"
: "${TRAFFIC_PROFILE:=realistic-mix-v1}"
: "${LOSS_MODEL:=iid}"
: "${TEST_RATE_BPS:=5000000}"
: "${TEST_DURATION_SEC:=45}"

(( LOSS_PCT >= 0 && LOSS_PCT <= 100 )) || { echo "LOSS_PCT must be 0..100" >&2; exit 2; }
[[ "$LOSS_MODEL" == iid ]] || { echo "constant reference LOSS_MODEL must be iid" >&2; exit 2; }
[[ "$TRAFFIC_PROFILE" == realistic-mix-v1 ]] || { echo "constant reference requires realistic-mix-v1" >&2; exit 2; }
...
RUNNER="$TMP_ROOT/validate-singlelane-constant${LOSS_PCT}-instrumented.sh"
cp "$BASE" "$RUNNER"
python3 - "$RUNNER" <<'PY'
...
if s.count(marker) != 1:
    raise SystemExit('constant wrapper: validator insertion marker drift')
...
out={
 ...
 'loss_pct_each_direction':int(loss),'rate_bps_each_direction':5000000,'duration_sec':45,
...
print('WBD_CONSTANT_RESULT '+json.dumps(out,sort_keys=True))
if root and os.path.isdir(root):
    open(os.path.join(root,'constant-result.json'),'w').write(json.dumps(out,sort_keys=True,indent=2)+'\n')
PY
