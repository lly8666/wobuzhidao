#!/usr/bin/env bash
set -uo pipefail
PRODUCT_DIR=${1:?usage: run_transient_profile_300ms_v3.sh PRODUCT_DIR HELPER_DIR}
...
case "$TEST_RATE_BPS" in
  ...
  *) echo "unsupported TEST_RATE_BPS=$TEST_RATE_BPS" >&2; exit 2 ;;
esac

src="${GITHUB_WORKSPACE:?}/suite/.github/scripts/run_transient_profile_300ms.sh"
patched="${RUNNER_TEMP:?}/run_transient_profile_300ms_rate_generic.sh"
python3 - "$src" "$patched" <<'PY'
from pathlib import Path
import sys
src,dst=map(Path,sys.argv[1:])
s=src.read_text()
old='''case "$TEST_RATE_BPS" in
  20000000) RATE_LABEL=20m ;;
  30000000) RATE_LABEL=30m ;;
  *) echo "TEST_RATE_BPS must be 20000000 or 30000000" >&2; exit 2 ;;
esac'''
new='''case "$TEST_RATE_BPS" in
  5000000) RATE_LABEL=5m ;;
  10000000) RATE_LABEL=10m ;;
  15000000) RATE_LABEL=15m ;;
  20000000) RATE_LABEL=20m ;;
  30000000) RATE_LABEL=30m ;;
  *) echo "TEST_RATE_BPS must be one of 5000000,10000000,15000000,20000000,30000000" >&2; exit 2 ;;
esac'''
if s.count(old) != 1:
    raise SystemExit(f'rate whitelist marker drift: {s.count(old)}')
dst.write_text(s.replace(old,new,1))
PY
chmod +x "$patched"

set +e
bash "$patched" "$PRODUCT_DIR" "$HELPER_DIR"
run_rc=$?
set -e
...
print('WBD_TRANSIENT_PROFILE_AUDIT_V3 '+json.dumps(d,sort_keys=True))
PY
else
