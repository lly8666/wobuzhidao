...
[[ "$LOSS_PCT" == 30 ]] || { echo "constant reference loss must be 30" >&2; exit 2; }
...
if s.count(marker) != 1:
    raise SystemExit('constant30 wrapper: validator insertion marker drift')
...
print('WBD_CONSTANT30_RESULT '+json.dumps(out,sort_keys=True))
if root and os.path.isdir(root):
    open(os.path.join(root,'constant30-result.json'),'w').write(json.dumps(out,sort_keys=True,indent=2)+'\n')
PY
