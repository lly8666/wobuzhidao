...
run_firewall() {
    action=$1
    if [ -n "$NFT_INPUT" ]; then
        sh "$FIREWALL_SCRIPT" "$action" --backend "$BACKEND" --front-port "$FRONT_PORT" --raw-port "$RAW_PORT" --state "$STATE" --nft-input "$NFT_INPUT"
    else
        sh "$FIREWALL_SCRIPT" "$action" --backend "$BACKEND" --front-port "$FRONT_PORT" --raw-port "$RAW_PORT" --state "$STATE"
    fi
}
