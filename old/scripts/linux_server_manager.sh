...
run_server() {
    ...
    set -- "$PREFIX/bin/wbd-ip-gateway-shared" -listen "$WBD_SHARED_TUN_LISTEN" \
        ...
        -tun-if "$WBD_SHARED_TUN_IF" -mtu "$WBD_MTU" -idle-timeout "$WBD_SHARED_TUN_IDLE" -max-sessions "$WBD_MAX_SESSIONS"
    ...
    "$PREFIX/bin/wbd-link-server-mux" -listen "$WBD_LINK_LISTEN" -service "$WBD_GAME_LISTEN" -raw-ip-service "$WBD_SHARED_TUN_LISTEN" -ticket-dir "$RUN/tickets" -ticket-ttl "$WBD_TICKET_TTL" -max-sessions "$WBD_MAX_SESSIONS" -mtu "$WBD_MTU" -connection-mtu "$WBD_CONNECTION_MTU" & pids="$pids $!"
