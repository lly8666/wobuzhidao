...
func TestTimestampedLogPathLivesUnderPortableLogsDirectory(t *testing.T) {
	...
	if filepath.Dir(path) != wantDir {
		t.Fatalf("log dir=%q want=%q", filepath.Dir(path), wantDir)
	}
...
func TestInstalledRuntimeUsesSiblingFilesWithoutExtraction(t *testing.T) {
	...
	if err := validateInstalledRuntime(dir); err != nil {
		t.Fatal(err)
	}
	if err := os.Remove(filepath.Join(dir, "wbd-link-proxy.exe")); err != nil {
		t.Fatal(err)
	}
