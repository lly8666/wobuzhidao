...
func gameControlResponder(t *testing.T, requests int) (string, <-chan gamelane.LaneSetCommand) {
	...
	go func() {
		...
		for i := 0; i < requests; i++ {
			...
			seen <- cmd
			active := make([]uint8, len(cmd.Lanes))
			for j, lane := range cmd.Lanes {
				active[j] = lane.ID
			}
...
func setControllerGameControl(c *Controller, addr string) {
	c.mu.Lock()
	c.gameControl = addr
	c.mu.Unlock()
}
...
func TestControllerDormantWakeKeepsSharedGameTunAndNetwork(t *testing.T) {
	...
	for _, ev := range r.events[cut:] {
		if ev == "stop:game" || ev == "stop:tun" || strings.HasPrefix(ev, "run:route-") || strings.HasPrefix(ev, "run:ipv6-") {
			t.Fatalf("dormant disturbed shared network: %v", r.events[cut:])
		}
	}
	first := <-requests
	if len(first.Lanes) != 0 {
		t.Fatalf("dormant Game lanes=%v", first.Lanes)
	}
	...
	for _, ev := range r.events[wakeCut:] {
		if ev == "start:game" || ev == "start:tun" || ev == "run:route-apply" || ev == "run:ipv6-apply" {
			t.Fatalf("wake restarted shared network: %v", r.events[wakeCut:])
		}
	}
	if got := c.executor.DynamicLaneIDs(); !reflect.DeepEqual(got, []int{1, 2}) {
		t.Fatalf("wake lanes=%v", got)
	}
}
...
func TestControllerReplaceLaneUsesBoundedMBBRaceAndAlternatesTransportSlot(t *testing.T) {
	...
	after := c.lifecycle.Snapshot()[0].Ref
	c.mu.Unlock()
	if firstPlan.Slot != 5 {
		t.Fatalf("first replacement slot=%d", firstPlan.Slot)
	}
	if after.Generation <= before.Generation {
		t.Fatalf("generation before=%+v after=%+v", before, after)
	}
	oldStop := []string{"stop:link-1", "stop:dtls-1", "stop:faketcp-1"}
	if !containsOrderedSubsequence(r.events[cut:], oldStop) {
		t.Fatalf("old lane was not retired after promotion: %v", r.events[cut:])
	}
	...
	c.mu.Unlock()
	if secondPlan.Slot != 1 {
		t.Fatalf("second replacement slot=%d", secondPlan.Slot)
	}
}

func TestControllerReplaceFourthLaneUsesPrivateSlotWithoutFifthLogicalLane(t *testing.T) {
	r := &recordingRunner{}
	c := testController(r)
	p := testProfile()
	p.TunnelIPv4 = ""
	...
	if len(promotion.Lanes) != 4 {
		t.Fatalf("promotion lanes=%v", promotion.Lanes)
	}
	if got := promotion.Lanes[3]; got.ID != 4 || got.Address != "127.0.0.1:47105" {
		t.Fatalf("lane4 candidate target=%+v", got)
	}
	if got := c.executor.DynamicLaneIDs(); !reflect.DeepEqual(got, []int{1, 2, 3, 4}) {
		t.Fatalf("slot5 leaked into logical lane set=%v", got)
	}
	c.mu.Lock()
	plan := c.lanePlans[4]
	after := c.lifecycle.Snapshot()[3].Ref
	c.mu.Unlock()
	if plan.Slot != makeBeforeBreakCandidateSlot || after.ID != 4 || after.Generation <= before.Generation {
		t.Fatalf("lane4 replacement plan=%+v before=%+v after=%+v", plan, before, after)
	}
}
...
func TestControllerReplaceCandidateFailureLeavesOldLaneAndGameUntouched(t *testing.T) {
	...
	if gotPlan.Slot != oldPlan.Slot || gotRef != oldRef || state != RuntimeConnected {
		t.Fatalf("old authority changed plan=%+v ref=%+v state=%s", gotPlan, gotRef, state)
	}
...
func TestControllerReplaceRejectsInactiveLane(t *testing.T) {
	r := &recordingRunner{}
	c := testController(r)
	p := testProfile()
	p.TunnelIPv4 = ""
	...
	err := c.ReplaceLane(2)
	if err == nil || !strings.Contains(err.Error(), "logical lane 2 is not active") {
		t.Fatalf("ReplaceLane(2) error=%v", err)
	}
}

func containsOrderedSubsequence(events, want []string) bool {
	j := 0
	for _, ev := range events {
		if j < len(want) && ev == want[j] {
			j++
		}
	}
	return j == len(want)
...
func TestGameControlResponderDoesNotHang(t *testing.T) {
	...
	if err := setGameLaneTargets(addr, nil, time.Second); err != nil {
		t.Fatal(err)
	}
	select {
	case <-seen:
	case <-time.After(time.Second):
		t.Fatal("control request not observed")
	}
}
