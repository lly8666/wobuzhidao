...
const defaultGameControlPort = 48102
...
func sameStringSet(a,b []string) bool {
	if len(a)!=len(b){return false};counts:=make(map[string]int,len(a));for _,s:=range a{counts[s]++};for _,s:=range b{if counts[s]==0{return false};counts[s]--};for _,n:=range counts{if n!=0{return false}};return true
}
...
func BuildMultiLanePlan(profile Profile, bootstraps []LaneBootstrap) (MultiLanePlan, error) {
	...
	for i:=range bootstraps{
		...
		if i==0{if err:=b.ValidateAuthenticated(nil);err!=nil{return MultiLanePlan{},fmt.Errorf("lane %d: %w",b.ID,err)};tunnel=b.TunnelConfig;continue}
		if err:=b.ValidateAuthenticated(&tunnel);err!=nil{return MultiLanePlan{},err}
	}

	profile.TunnelIPv4=tunnel.Address4
	common,err:=BuildPlan(profile,bootstraps[0].Underlay,bootstraps[0].Ticket);if err!=nil{return MultiLanePlan{},err}
	...
	for _,b:=range bootstraps{
		...
		// profile.MTU is the inner/Wintun IP-packet MTU. wbd-tun first adds
		// the fixed WBDP dataplane header, then Game adds its WGL1 envelope.
		// Initial authoritative lanes and replacement candidates must budget
		// the same complete plaintext size before LINK.
...
func (p MultiLanePlan) ProcessCommands() []Command {
	out:=make([]Command,0,len(p.Lanes)*3+2);for _,lane:=range p.Lanes{out=append(out,lane.FakeTCP,lane.DTLS,lane.Link)};out=append(out,p.Game,p.TUN);return out
}
