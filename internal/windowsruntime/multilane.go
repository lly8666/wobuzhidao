...
func lanePort(base, laneID int) (int, error) {
	if laneID < 1 || laneID > logicaltunnel.MaxProductPublicTransportLanes { return 0, logicaltunnel.ErrTransportLanes }
...
func BuildLaneBootstrap(profile Profile, base Underlay, laneID int) (LaneBootstrap, error) {
	...
	configPath, err := laneStatePath(profile.TunnelConfigPath, laneID); if err != nil { return LaneBootstrap{}, err }
	if base.SourcePort == 0 { return LaneBootstrap{}, errors.New("lane bootstrap requires an assigned dynamic FakeTCP source port") }
	...
	return LaneBootstrap{
		ID: laneID, Underlay: base,
		FakeTCP: Command{Name: fmt.Sprintf("faketcp-%d", laneID), Path: filepath.Join(profile.BinDir, "wbd-faketcp.exe"), Args: args},
		TicketPath: ticketPath, TunnelConfigPath: configPath,
	}, nil
}

func (b LaneBootstrap) ValidateAuthenticated(expected *logicaltunnel.TunnelConfig) error {
	if b.ID < 1 || b.ID > logicaltunnel.MaxProductPublicTransportLanes { return logicaltunnel.ErrTransportLanes }
	if err := b.Underlay.Validate(); err != nil { return err }
	if len(strings.TrimSpace(b.Ticket)) != 64 { return errors.New("lane Reality ticket must be 64 hex characters") }
	for _, c := range b.Ticket { if !strings.ContainsRune("0123456789abcdefABCDEF", c) { return errors.New("lane Reality ticket must be hexadecimal") } }
	if err := b.TunnelConfig.Validate(); err != nil { return err }
	if expected != nil {
		if b.TunnelConfig.TunnelID != expected.TunnelID || b.TunnelConfig.Address4 != expected.Address4 || !sameStringSet(b.TunnelConfig.Routes4, expected.Routes4) {
...
func sameStringSet(a, b []string) bool {
	...
	for _, s := range b { if counts[s] == 0 { return false }; counts[s]-- }
...
func BuildMultiLanePlan(profile Profile, bootstraps []LaneBootstrap) (MultiLanePlan, error) {
	profile = profile.normalized()
	if err := profile.Validate(); err != nil { return MultiLanePlan{}, err }
	...
	for i := range bootstraps {
		b := bootstraps[i]
		if i == 0 {
			if err := b.ValidateAuthenticated(nil); err != nil { return MultiLanePlan{}, fmt.Errorf("lane %d: %w", b.ID, err) }
			tunnel = b.TunnelConfig
		} else if err := b.ValidateAuthenticated(&tunnel); err != nil { return MultiLanePlan{}, err }
		if b.ID != i+1 { return MultiLanePlan{}, fmt.Errorf("lane ordering mismatch: index=%d id=%d", i, b.ID) }
	}
	profile.TunnelIPv4 = tunnel.Address4
	common, err := BuildPlan(profile, bootstraps[0].Underlay, bootstraps[0].Ticket)
	if err != nil { return MultiLanePlan{}, err }
	...
	for _, b := range bootstraps {
		fakeUDP, _ := laneLoopback(defaultFakeTCPLocalPort, b.ID)
		...
		lanes = append(lanes, LanePlan{
			ID: b.ID,
			FakeTCP: b.FakeTCP,
			DTLS: Command{Name: fmt.Sprintf("dtls-%d", b.ID), Path: bin("wbd_dtls_shim.exe"), Args: []string{"client", strconv.Itoa(dtlsPort), "127.0.0.1", strconv.Itoa(fakePort), "none", "none"}},
			Link: Command{Name: fmt.Sprintf("link-%d", b.ID), Path: bin("wbd-link-proxy.exe"), Args: []string{"-mode", "client", "-listen", linkListen, "-dtls", dtlsPlain, "-fec", profile.FEC, "-mtu", strconv.Itoa(profile.MTU), "-lanes", "1", "-demo-reality-ticket", strings.TrimSpace(b.Ticket)}},
		})
		_ = fakeUDP
	...
	game := Command{
		Name: "game", Path: bin("wbd-game-lane-client.exe"),
		Args: []string{"-listen", gameListen, "-lanes", strings.Join(linkAddresses, ","), "-session-id", string(tunnel.TunnelID), "-max-reorder", "4096", "-idle", "90s"},
	}
	tun := common.TUN
	for i := 0; i+1 < len(tun.Args); i++ { if tun.Args[i] == "-transport" { tun.Args[i+1] = gameListen } }
	return MultiLanePlan{
		Lanes: lanes, Game: game, TUN: tun,
		IPv6Apply: common.IPv6Apply, RouteApply: common.RouteApply, RouteCleanup: common.RouteCleanup, IPv6Cleanup: common.IPv6Cleanup,
		TunnelConfig: tunnel,
	}, nil
}
...
func (p MultiLanePlan) ProcessCommands() []Command {
	...
	for _, lane := range p.Lanes { out = append(out, lane.FakeTCP, lane.DTLS, lane.Link) }
