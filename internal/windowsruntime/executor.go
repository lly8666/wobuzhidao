...
func (e *Executor) StartAfterFakeTCP(plan Plan, fake Process) error {
	if fake == nil {
		return errors.New("prestarted FakeTCP process is required")
	}
	e.mu.Lock()
	defer e.mu.Unlock()
...
func (e *Executor) startLocked(plan Plan, prestartedFake Process) error {
	if err := e.ensureStartableLocked(); err != nil {
		return err
	}
	...
	if err := e.applyNetworkLocked(plan); err != nil {
		return err
	}
	e.plan = plan
	e.running = true
...
func (e *Executor) StartMultiLane(plan MultiLanePlan, prestartedFake map[int]Process) error {
	...
	if len(plan.Lanes) == 0 {
		return errors.New("multi-lane plan requires at least one lane")
	}
	if len(prestartedFake) != len(plan.Lanes) {
		return fmt.Errorf("prestarted FakeTCP lanes=%d want=%d", len(prestartedFake), len(plan.Lanes))
	}
	...
	for _, lane := range plan.Lanes {
		...
		if fake == nil {
			e.rollbackProcessesLocked()
			return fmt.Errorf("prestarted FakeTCP lane %d is missing", lane.ID)
		}
		e.processes = append(e.processes, namedProcess{name: lane.FakeTCP.Name, proc: fake})
		if err := waitProcessReady(lane.FakeTCP.Name, fake); err != nil {
			e.rollbackProcessesLocked()
			return err
		}
	...
	if err := e.applyNetworkLocked(cleanupPlan); err != nil {
		return err
	}
	e.plan = cleanupPlan
	e.running = true
...
// Start/Stop network mutation. Script failure is non-destructive by contract: it
// leaves the previous route ownership authoritative, so the shared runtime keeps
// running and the controller can retry from a fresh path observation.
func (e *Executor) RebindRoutes(command Command) error {
	...
	if !e.running {
		return errors.New("Windows runtime is not running")
	}
	if err := e.runner.Run(command); err != nil {
		return fmt.Errorf("rebind Windows physical routes: %w", err)
	}
	return nil
}
...
func (e *Executor) Stop() error {
	...
	if !e.running && len(e.processes) == 0 {
		...
		if err := e.runner.Run(e.plan.RouteCleanup); err != nil {
			retryErrs = append(retryErrs, fmt.Errorf("cleanup capture routes: %w", err))
		}
		if err := e.runner.Run(e.plan.IPv6Cleanup); err != nil {
			retryErrs = append(retryErrs, fmt.Errorf("cleanup IPv6 kill switch: %w", err))
		}
		if len(retryErrs) != 0 {
	...
	if routeCleanupErr != nil {
		errs = append(errs, fmt.Errorf("cleanup capture routes: %w", routeCleanupErr))
	}
	ipv6CleanupErr := e.runner.Run(e.plan.IPv6Cleanup)
	if ipv6CleanupErr != nil {
		errs = append(errs, fmt.Errorf("cleanup IPv6 kill switch: %w", ipv6CleanupErr))
	}
...
func (p *osProcess) WaitReady(marker string, timeout time.Duration) error {
	...
	for {
		...
		p.mu.Lock()
		exited, err := p.exited, p.err
		p.mu.Unlock()
		if exited {
			if err == nil {
				return fmt.Errorf("process exited before readiness marker %q", marker)
			}
...
func (p *osProcess) Stop() error {
	...
	if isBenignProcessStopError(err) {
		return nil
	}
	return err
}
