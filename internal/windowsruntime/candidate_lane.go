...
func BuildCandidateLaneBootstrapSlot(profile Profile, base Underlay, laneID, slot int) (LaneBootstrap, error) {
	...
	localUDP,err:=transportSlotLoopback(defaultFakeTCPLocalPort,slot);if err!=nil{return LaneBootstrap{},err}
	...
	return LaneBootstrap{ID:laneID,Underlay:base,FakeTCP:Command{Name:fmt.Sprintf("faketcp-%d-candidate-s%d",laneID,slot),Path:filepath.Join(profile.BinDir,"wbd-faketcp.exe"),Args:args},TicketPath:ticketPath,TunnelConfigPath:configPath},nil
}
