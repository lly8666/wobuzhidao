...
func decodeAuthenticatedTunnelConfig(raw string) (logicaltunnel.TunnelConfig, error) {
	...
	if err := dec.Decode(&cfg); err != nil {
		return cfg, fmt.Errorf("decode authenticated tunnel config: %w", err)
	}
	if err := cfg.Validate(); err != nil {
		return cfg, fmt.Errorf("validate authenticated tunnel config: %w", err)
	}
	return cfg, nil
}
...
func (c *Controller) Connect(profile Profile) error {
	...
	baseUnderlay, err := c.discoverer.Discover(profile)
	if err != nil {
	...
	defer func() {
		...
		for laneID := profile.Lanes; laneID >= 1; laneID-- {
			if proc := prestarted[laneID]; proc != nil {
				_ = proc.Stop()
			}
		}
	}()
	...
	for laneID := 1; laneID <= profile.Lanes; laneID++ {
		...
		bootstrap, err := BuildLaneBootstrap(profile, laneUnderlay, laneID)
		if err != nil {
			return fmt.Errorf("build lane %d bootstrap: %w", laneID, err)
		}
		if err := c.tickets.Clear(bootstrap.TicketPath); err != nil {
			return fmt.Errorf("clear lane %d Reality ticket: %w", laneID, err)
		}
		if err := c.tickets.Clear(bootstrap.TunnelConfigPath); err != nil {
			return fmt.Errorf("clear lane %d authenticated tunnel config: %w", laneID, err)
		}

		proc, err := c.runner.Start(bootstrap.FakeTCP)
		if err != nil {
			return fmt.Errorf("start lane %d same-flow FakeTCP: %w", laneID, err)
		}
		...
		if err != nil {
			return fmt.Errorf("read lane %d Reality ticket after bootstrap readiness: %w", laneID, err)
		}
		rawTunnelConfig, err := c.tickets.Read(bootstrap.TunnelConfigPath)
		if err != nil {
			return fmt.Errorf("read lane %d authenticated tunnel config after bootstrap readiness: %w", laneID, err)
		}
		tunnelConfig, err := decodeAuthenticatedTunnelConfig(rawTunnelConfig)
		if err != nil {
			return fmt.Errorf("lane %d: %w", laneID, err)
		}
		bootstrap.Ticket = ticket
		bootstrap.TunnelConfig = tunnelConfig
		if laneID > 1 {
			if err := bootstrap.ValidateAuthenticated(&bootstraps[0].TunnelConfig); err != nil {
				return err
			}
		}
	...
	multi, err := BuildMultiLanePlan(profile, bootstraps)
	if err != nil {
		return fmt.Errorf("build Windows multi-lane runtime plan: %w", err)
	}
	profile.TunnelIPv4 = multi.TunnelConfig.Address4

	owned = false
	if err := c.executor.StartMultiLane(multi, prestarted); err != nil {
		return err
	}
	...
	for _, lane := range multi.Lanes {
		if _, err := lifecycle.AttachInitial(uint8(lane.ID)); err != nil {
			_ = c.executor.Stop()
			return err
		}
		plans[lane.ID] = lane
	...
	c.mu.Lock()
	c.profile = profile
	c.baseUnderlay = baseUnderlay
	c.tunnelConfig = multi.TunnelConfig
	c.gameControl = multi.GameControl
	c.lanePlans = plans
...
func (c *Controller) clearRuntimeContextLocked() {
	c.stopPayloadIdleMonitorLocked()
	c.profile = Profile{}
	c.baseUnderlay = Underlay{}
	c.tunnelConfig = logicaltunnel.TunnelConfig{}
	c.gameControl = ""
	c.lanePlans = nil
	c.lifecycle = nil
}
...
func (FileTicketStore) Read(path string) (string, error) {
	...
	for {
		...
		if err == nil {
			...
			last = errors.New("state file is empty")
		} else {
			last = err
		}
		if time.Now().After(deadline) {
			return "", fmt.Errorf("state readiness timeout: %w", last)
		}
