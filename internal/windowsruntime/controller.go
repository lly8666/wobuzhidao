...
func (c *Controller) recoverStaleNetworkState(profile Profile) error {
	for _, command := range startupRecoveryCommands(profile) {
		if err := c.runner.Run(command); err != nil { return fmt.Errorf("startup %s: %w", command.Name, err) }
	}
	return nil
}
...
func (c *Controller) Connect(profile Profile) error {
	...
	c.mu.Lock()
	if c.state != RuntimeDisconnected { state:=c.state;c.mu.Unlock();return fmt.Errorf("Windows runtime cannot connect while %s",state) }
	...
	if err:=c.tickets.Clear(profile.TunnelConfigPath);err!=nil{return fmt.Errorf("clear stale authenticated tunnel config: %w",err)}

	underlay,err:=c.discoverer.Discover(profile);if err!=nil{return fmt.Errorf("discover Windows FakeTCP underlay: %w",err)}
	if underlay.SourcePort==0{underlay.SourcePort=nextFakeTCPSourcePort()}
	fakeCommand,err:=BuildFakeTCPCommand(profile,underlay);if err!=nil{return fmt.Errorf("build single-flow FakeTCP command: %w",err)}
	fakeProc,err:=c.runner.Start(fakeCommand);if err!=nil{return fmt.Errorf("start single-flow FakeTCP: %w",err)}
	fakeOwned:=true
	defer func(){if fakeOwned{_ = fakeProc.Stop()}}()

	if err:=waitProcessMarker("single-flow Reality bootstrap",fakeProc,singleFlowBootstrapReadyMarker,singleFlowBootstrapWait);err!=nil{return err}
	ticket,err:=c.tickets.Read(profile.TicketPath);if err!=nil{return fmt.Errorf("read single-flow Reality ticket after bootstrap readiness: %w",err)}
	rawTunnelConfig,err:=c.tickets.Read(profile.TunnelConfigPath);if err!=nil{return fmt.Errorf("read authenticated tunnel config after bootstrap readiness: %w",err)}
	tunnelConfig,err:=decodeAuthenticatedTunnelConfig(rawTunnelConfig);if err!=nil{return err}
	// The server-authenticated /32 is the route/address authority. User profile
	// tunnel_ipv4 compatibility fields never override this lease.
	profile.TunnelIPv4 = tunnelConfig.Address4
	plan,err:=BuildPlan(profile,underlay,ticket);if err!=nil{return fmt.Errorf("build Windows runtime plan: %w",err)}

	fakeOwned=false
	if err:=c.executor.StartAfterFakeTCP(plan,fakeProc);err!=nil{return err}
	c.mu.Lock();c.state=RuntimeConnected;c.mu.Unlock();connected=true
	return nil
}
...
func (c *Controller) Disconnect() error {
	c.mu.Lock()
	switch c.state {
	case RuntimeDisconnected:c.mu.Unlock();return c.executor.Stop()
	case RuntimeConnected:c.state=RuntimeDisconnecting
	default:state:=c.state;c.mu.Unlock();return fmt.Errorf("Windows runtime cannot disconnect while %s",state)
	}
	c.mu.Unlock()
...
func (FileTicketStore) Read(path string)(string,error){
	deadline:=time.Now().Add(15*time.Second);var last error
	for{b,err:=os.ReadFile(path);if err==nil{body:=strings.TrimSpace(string(b));if body!=""{return body,nil};last=errors.New("state file is empty")}else{last=err};if time.Now().After(deadline){return "",fmt.Errorf("state readiness timeout: %w",last)};time.Sleep(25*time.Millisecond)}
}
