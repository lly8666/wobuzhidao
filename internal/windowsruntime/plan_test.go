...
func TestBuildPlanUsesSingleFlowWindowsStack(t *testing.T) {
	...
	if !argPair(p.FakeTCP.Args, "--reality-server-name", testProfile().ServerName) || !argPair(p.FakeTCP.Args, "--reality-route-key", testProfile().RouteKey) || !argPair(p.FakeTCP.Args, "--reality-username", testProfile().Username) || !argPair(p.FakeTCP.Args, "--reality-password", testProfile().Password) || !argPair(p.FakeTCP.Args, "--reality-ticket-out", testProfile().TicketPath) {
		t.Fatalf("Reality bootstrap must be inside FakeTCP command: %v", p.FakeTCP.Args)
	}
