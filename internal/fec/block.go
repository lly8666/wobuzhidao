...
var (
	...
	ErrShardMismatch  = errors.New("fec: conflicting duplicate shard")
)
...
func (d *BlockDecoder) addStreamingSource(b *decodeBlock, h BlockHeader, payload []byte) ([][]byte, bool, error) {
	...
	if b.final {
		...
		if b.present[idx] {
			if !bytes.Equal(b.shards[idx][:len(payload)], payload) {
				return nil, false, ErrShardMismatch
			}
			return nil, false, nil
		}
		...
		if b.sourcePresent[idx] {
			if !bytes.Equal(b.sources[idx], payload) {
				return nil, false, ErrShardMismatch
			}
			return nil, false, nil
		}
...
func (d *BlockDecoder) addFinalShard(b *decodeBlock, h BlockHeader, payload []byte) ([][]byte, bool, error) {
	...
	idx := int(h.ShardIndex)
	if b.present[idx] {
		if !bytes.Equal(b.shards[idx], payload) {
			return nil, false, ErrShardMismatch
		}
		return nil, false, nil
	}
