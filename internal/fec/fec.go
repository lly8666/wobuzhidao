...
var fixedParityProfiles = [...]int{4, 8, WeakParityShards, 12, 16, ParityShards}

// SupportedParityShards returns the complete fixed FEC family admitted by the
// archived live LINK policy. The returned slice is a copy and may be modified.
func SupportedParityShards() []int {
	return append([]int(nil), fixedParityProfiles[:]...)
}

// IsSupportedParityShards reports whether n is one of the live fixed 20:R
// profiles. FEC off is represented by the LINK/path layer as R=0 and is not a
// Reed-Solomon geometry.
func IsSupportedParityShards(n int) bool {
	for _, allowed := range fixedParityProfiles {
		if n == allowed {
			return true
		}
	}
...
func validParityCount(n int) bool { return IsSupportedParityShards(n) }
...
func ParseBlockHeader(b []byte) (BlockHeader, error) {
	...
	if b[0] != 'W' || b[1] != 'F' || b[2] != HeaderVersion || b[3] != 0 {
		return h, errors.New("fec: invalid header magic/version/reserved")
	}
