...
func TestProductTransportLanePolicyIsExactlyOne(t *testing.T) {
	if MinProductPublicTransportLanes != 1 || MaxProductPublicTransportLanes != 1 {
		t.Fatalf("ADR-0014 transport bounds changed: min=%d max=%d", MinProductPublicTransportLanes, MaxProductPublicTransportLanes)
	}
	if err := ValidateProductTransportLaneCount(1); err != nil {
		t.Fatalf("single product transport rejected: %v", err)
	}
	for _, n := range []int{-1, 0, 2, 4, 5, 8} {
...
func TestSameAccountInstallationsReceiveDistinctLeases(t *testing.T) {
	...
	if a.Config.TunnelID == b.Config.TunnelID {
		t.Fatal("distinct installations reused TunnelID")
	}
	if a.Config.Address4 == b.Config.Address4 {
		t.Fatalf("distinct installations reused lease %s", a.Config.Address4)
	}
...
func TestValidateIPv4SourceRejectsOtherTunnelLease(t *testing.T) {
	...
	if err := ValidateIPv4Source(packet, aIP); !errors.Is(err, ErrSourceSpoof) {
		t.Fatalf("spoofed B source accepted for A lease: %v", err)
	}
	if err := ValidateIPv4Source(packet, bIP); err != nil {
		t.Fatalf("owner source rejected: %v", err)
	}
}
