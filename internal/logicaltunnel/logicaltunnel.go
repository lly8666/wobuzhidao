...
func ParseInstallationID(s string) (InstallationID, error) {
	...
	if err != nil || len(b) != InstallationIDBytes {
...
func ParseTunnelID(s string) (TunnelID, error) {
	s = strings.ToLower(strings.TrimSpace(s))
	b, err := hex.DecodeString(s)
	if err != nil || len(b) != TunnelIDBytes {
		return "", ErrInvalidIdentity
	}
...
func (c TunnelConfig) Validate() error {
	if _, err := ParseTunnelID(string(c.TunnelID)); err != nil {
		return err
	}
...
func identityKey(account string, installation InstallationID) (string, error) {
	...
	id, err := ParseInstallationID(string(installation))
	if err != nil {
		return "", err
	}
	return account + "\x00" + string(id), nil
}
...
func (m *Manager) Lookup(tunnelID TunnelID) (Lease, error) {
	...
	id, err := ParseTunnelID(string(tunnelID))
	if err != nil {
		return Lease{}, err
	}
	m.mu.Lock()
...
func cloneLease(in *Lease) Lease {
	...
	out := *in
	out.Config.Routes4 = append([]string(nil), in.Config.Routes4...)
	return out
}
...
func ValidateIPv4Source(packet []byte, leased netip.Addr) error {
	...
	if err := dataplane.ValidateIPPacket(packet); err != nil {
		return err
	}
	if packet[0]>>4 != 4 {
		return ErrSourceSpoof
