...
func (l *LaneLifecycle) AttachInitial(id uint8) (LaneRef, error) {
	if l == nil || id == 0 {
		return LaneRef{}, ErrLaneState
	}
	if !ValidProductLaneID(id) {
		return LaneRef{}, ErrTransportLanes
	}
	if _, ok := l.lanes[id]; ok {
		return LaneRef{}, ErrLaneExists
	}
	if len(l.lanes) >= l.max {
		return LaneRef{}, ErrTransportLanes
	}
	ref := l.newRef(id)
...
func (l *LaneLifecycle) BeginReplacement(old LaneRef, candidateID uint8) (LaneRef, error) {
	...
	if oldRec.phase != LaneActive {
		return LaneRef{}, ErrLaneState
	}
	if candidateID == 0 || candidateID == old.ID {
		return LaneRef{}, ErrLaneState
	}
	if !ValidProductLaneID(candidateID) {
		return LaneRef{}, ErrTransportLanes
	}
	if _, ok := l.lanes[candidateID]; ok {
		return LaneRef{}, ErrLaneExists
	}
	if len(l.lanes) >= l.max {
		return LaneRef{}, ErrTransportLanes
...
func (l *LaneLifecycle) CandidateHealthy(candidate LaneRef) error {
	...
	if rec.phase != LaneCandidate || rec.replaces == nil {
		return ErrLaneState
	}
	if _, err := l.current(*rec.replaces); err != nil {
		return err
	}
...
func (l *LaneLifecycle) PromoteSameIDReplacement(old LaneRef) (LaneRef, error) {
	...
	rec, err := l.current(old)
	if err != nil {
		return LaneRef{}, err
	}
	if rec.phase != LaneActive {
		return LaneRef{}, ErrLaneState
	}
	fresh := l.newRef(old.ID)
	rec.ref = fresh
	rec.phase = LaneActive
	rec.replaces = nil
	return fresh, nil
...
func (l *LaneLifecycle) current(ref LaneRef) (*laneRecord, error) {
	if ref.ID == 0 || ref.Generation == 0 {
		return nil, ErrLaneState
	}
	rec := l.lanes[ref.ID]
	if rec == nil {
		return nil, ErrLaneNotFound
	}
	if rec.ref.Generation != ref.Generation {
		return nil, fmt.Errorf("%w: lane=%d got=%d current=%d", ErrStaleLaneGeneration, ref.ID, ref.Generation, rec.ref.Generation)
	}
	return rec, nil
}
