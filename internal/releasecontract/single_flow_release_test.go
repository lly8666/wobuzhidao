...
func TestLinuxProductRunPathSupportsTunnelRaceAboveOnePublicRawMux(t *testing.T) {
	...
	requireContains(t, run, `-service "$WBD_PLATFORM_LISTEN"`, "Linux Game downstream")
	requireContains(t, run, `-service "$WBD_GAME_LISTEN"`, "Linux LINK-to-Game wiring")
...
func TestLinuxBundleCarriesSubstantiveSourceSHAAndMultipathContract(t *testing.T) {
	...
	requireContains(t, builder, "game_product=1", "Linux release evidence")
