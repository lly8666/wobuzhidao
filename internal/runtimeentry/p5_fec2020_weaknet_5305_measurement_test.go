...
func p5FEC2020Weak5305RunConfig(t *testing.T) (int, uint64) {
	t.Helper()
...
func TestP5FEC2020WeakNetwork5305MeasurementHarness(t *testing.T) {
	if os.Getenv("WBD_P5_FEC2020_WEAKNET_MEASURE") != "1" {
		t.Skip("P5 fixed-FEC 20:20 weak-network harness runs only in its dedicated Actions gate")
	}
	artifactDir := os.Getenv("WBD_P5_FEC2020_WEAKNET_ARTIFACT_DIR")
	if artifactDir == "" {
		t.Fatal("WBD_P5_FEC2020_WEAKNET_ARTIFACT_DIR is required")
	}
	...
	fecFlushAfter, fecMaxBlocks, err := datapath.FixedFECRuntimeDefaults(20)
	if err != nil {
		t.Fatal(err)
	}
	if fecFlushAfter != 8*time.Millisecond || fecMaxBlocks != 8 {
		t.Fatalf("fixed FEC 20:20 runtime defaults changed: flush=%s max_blocks=%d", fecFlushAfter, fecMaxBlocks)
	}
	...
	runOrdinal, seed := p5FEC2020Weak5305RunConfig(t)
	...
	server, err := NewServer(ServerConfig{
		...
		Lane:   datapath.ServerLaneParams{ConnectionMTU: 1500, TxIPv4HeaderLen: 20, TxTCPHeaderLen: 20, RxIPv4HeaderLen: 20, RxTCPHeaderLen: 20, ParityShards: 20, FlushAfter: fecFlushAfter, MaxBlocks: fecMaxBlocks},
	...
	client, err := DialClient(ctx, ClientConfig{
		...
		Lane:      datapath.ClientLaneParams{ConnectionMTU: 1500, TxIPv4HeaderLen: 20, TxTCPHeaderLen: 20, RxIPv4HeaderLen: 20, RxTCPHeaderLen: 20, ParityShards: 20, FlushAfter: fecFlushAfter, MaxBlocks: fecMaxBlocks},
	...
	if laneStats, ok := client.Owner().LaneStats(initialRef); !ok || !laneStats.TxPath.FECEnabled || !laneStats.RxPath.FECEnabled || laneStats.TxPath.ParityShards != 20 || laneStats.RxPath.ParityShards != 20 {
		t.Fatalf("weaknet fixed-FEC 20:20 client profile mismatch: ok=%v stats=%+v", ok, laneStats)
	}
	if laneStats, ok := serverTunnel.owner.LaneStats(serverTunnel.ref); !ok || !laneStats.TxPath.FECEnabled || !laneStats.RxPath.FECEnabled || laneStats.TxPath.ParityShards != 20 || laneStats.RxPath.ParityShards != 20 {
		t.Fatalf("weaknet fixed-FEC 20:20 server profile mismatch: ok=%v stats=%+v", ok, laneStats)
	}
	...
	if !clientLane.TxPath.FECEnabled || !serverLane.TxPath.FECEnabled ||
		!clientLane.RxPath.FECEnabled || !serverLane.RxPath.FECEnabled ||
		clientLane.TxPath.ParityShards != 20 || serverLane.TxPath.ParityShards != 20 ||
		clientLane.RxPath.ParityShards != 20 || serverLane.RxPath.ParityShards != 20 {
		t.Fatalf("weaknet fixed-FEC 20:20 profile changed client_tx=%+v client_rx=%+v server_tx=%+v server_rx=%+v",
			clientLane.TxPath, clientLane.RxPath, serverLane.TxPath, serverLane.RxPath)
	}
	if clientLane.TxPath.Encoder.SourceShards == 0 || serverLane.TxPath.Encoder.SourceShards == 0 ||
		clientLane.TxPath.Encoder.ParityShards == 0 || serverLane.TxPath.Encoder.ParityShards == 0 {
		t.Fatalf("weaknet fixed-FEC 20:20 emitted no shards client=%+v server=%+v", clientLane.TxPath.Encoder, serverLane.TxPath.Encoder)
	}
	...
	if err := weakArtifacts.writeResource(map[string]any{
		...
		"fec_profile": "20:20", "fec_data_shards": 20, "fec_parity_config": 20, "fec_flush_after_ns": fecFlushAfter.Nanoseconds(), "fec_max_blocks": fecMaxBlocks,
		...
		"client_full_blocks": clientLane.TxPath.Encoder.FullBlocks, "client_partial_blocks": clientLane.TxPath.Encoder.PartialBlocks, "client_pending_sources": clientLane.TxPath.Encoder.PendingSources,
		"client_decoder_in_flight": clientLane.RxPath.Decoder.InFlight, "client_decoder_max_blocks": clientLane.RxPath.Decoder.MaxBlocks, "client_decoder_retired": clientLane.RxPath.Decoder.Retired,
		...
		"client_recovery_pending_deadlines": clientLane.RxPath.Recovery.PendingDeadlines, "client_recovery_expire_events": clientLane.RxPath.Recovery.ExpireEvents,
		"client_recovery_expired_incomplete": clientLane.RxPath.Recovery.ExpiredIncomplete, "client_recovery_expired_missing_sources": clientLane.RxPath.Recovery.ExpiredMissingSources,
		"server_source_shards": serverLane.TxPath.Encoder.SourceShards, "server_parity_shards": serverLane.TxPath.Encoder.ParityShards,
		"server_full_blocks": serverLane.TxPath.Encoder.FullBlocks, "server_partial_blocks": serverLane.TxPath.Encoder.PartialBlocks, "server_pending_sources": serverLane.TxPath.Encoder.PendingSources,
		"server_decoder_in_flight": serverLane.RxPath.Decoder.InFlight, "server_decoder_max_blocks": serverLane.RxPath.Decoder.MaxBlocks, "server_decoder_retired": serverLane.RxPath.Decoder.Retired,
		...
		"server_recovery_pending_deadlines": serverLane.RxPath.Recovery.PendingDeadlines, "server_recovery_expire_events": serverLane.RxPath.Recovery.ExpireEvents,
		"server_recovery_expired_incomplete": serverLane.RxPath.Recovery.ExpiredIncomplete, "server_recovery_expired_missing_sources": serverLane.RxPath.Recovery.ExpiredMissingSources,
	...
	manifest := map[string]any{
		...
		"scenario": map[string]any{
			"name": "controlled-real-https-fec20x20-weaknet-5-30-5", "run_ordinal": runOrdinal, "seed": seed, "cryptographic_rng": "system",
			...
			"connection_mtu": 1500, "desired_lanes": 1, "fec_profile": "20:20", "fec_data_shards": 20, "fec_parity_shards": 20, "fec_flush_after_ns": fecFlushAfter.Nanoseconds(), "fec_max_blocks": fecMaxBlocks, "fec_profile_role": "explicit-controlled-weaknet-capability-cost-scenario-not-production-default", "production_default_fec": "off", "padding": "off",
		...
		"fec": map[string]any{
			"profile": "20:20",
			"client_source_shards": clientLane.TxPath.Encoder.SourceShards, "client_parity_shards": clientLane.TxPath.Encoder.ParityShards,
			"client_full_blocks": clientLane.TxPath.Encoder.FullBlocks, "client_partial_blocks": clientLane.TxPath.Encoder.PartialBlocks,
			"client_reconstruction_events": clientLane.RxPath.Decoder.ReconstructionEvents, "client_recovered_sources": clientLane.RxPath.Decoder.RecoveredSources,
			"client_expired_missing_sources": clientLane.RxPath.Recovery.ExpiredMissingSources,
			"server_source_shards": serverLane.TxPath.Encoder.SourceShards, "server_parity_shards": serverLane.TxPath.Encoder.ParityShards,
			"server_full_blocks": serverLane.TxPath.Encoder.FullBlocks, "server_partial_blocks": serverLane.TxPath.Encoder.PartialBlocks,
			"server_reconstruction_events": serverLane.RxPath.Decoder.ReconstructionEvents, "server_recovered_sources": serverLane.RxPath.Decoder.RecoveredSources,
			"server_expired_missing_sources": serverLane.RxPath.Recovery.ExpiredMissingSources,
		},
	...
	fmt.Printf("WBD_P5_FEC2020_WEAKNET_5305_CAPTURED source_sha=%s run=%d seed=%d duration=120s delay=300ms drop=5-30-5 fec=20:20 padding=off scheduled=%d success=%d loss=%d recovered=%d repair=%d wire_amp=%.6f\n", sourceSHA, runOrdinal, seed, totalScheduled, successes, totalScheduled-successes, clientLane.RxPath.Decoder.RecoveredSources+serverLane.RxPath.Decoder.RecoveredSources, repairRetransmits, wireAmplification)
}
