...
func TestLoadRuntimeProfileAppliesOnlyProductDefaults(t *testing.T) {
  ...
  "fec": "20:20"
	...
	if profile.FEC != "20:20" || profile.IfName != "WBD" || profile.MTU != 1400 || profile.RouteMode != "Full" || profile.TunnelIPv4 != "10.66.0.2/30" {
	...
	if !strings.HasSuffix(profile.TicketPath, filepath.Join("state", "reality-ticket.tmp")) ||
		...
		t.Fatalf("state paths escaped installed state dir: %+v", profile)
	}
}
