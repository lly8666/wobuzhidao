...
type RuntimeProfileFile struct {
	...
	ProxyLAN               *bool  `json:"proxy_lan,omitempty"`
	ProxyChina             *bool  `json:"proxy_china,omitempty"`
	ProxyOther             *bool  `json:"proxy_other,omitempty"`
...
func ensureInstallationID(stateDir string) (logicaltunnel.InstallationID, error) {
	if strings.TrimSpace(stateDir) == "" {
		return "", fmt.Errorf("Windows state directory is required")
	}
	if err := os.MkdirAll(stateDir, 0o700); err != nil {
		return "", fmt.Errorf("create Windows state directory: %w", err)
	}
	...
	read := func() (logicaltunnel.InstallationID, error) {
		...
		if err != nil {
			return "", fmt.Errorf("invalid persisted WBD installation id: %w", err)
		}
		return id, nil
	}
	...
	if err != nil {
		return "", fmt.Errorf("generate WBD installation id: %w", err)
	}
...
func LoadRuntimeProfile(path, binDir, stateDir string) (windowsruntime.Profile, error) {
	...
	serverFront, serverRaw, err := resolveServerEndpoints(cfg)
	if err != nil {
		return windowsruntime.Profile{}, err
	}
	routingPolicy, err := routingPolicyFromConfig(cfg)
	if err != nil {
		return windowsruntime.Profile{}, err
	}
	if cfg.FEC == "" {
	...
	if cfg.MTU == 0 {
		cfg.MTU = windowsruntime.DefaultTunnelMTU
	}
	if cfg.RouteMode == "" {
	...
	if cfg.LaneRotationMinSeconds != nil {
		laneRotationMinSeconds = *cfg.LaneRotationMinSeconds
	}
	laneRotationMaxSeconds := windowsruntime.DefaultLaneRotationMaxSeconds
	if cfg.LaneRotationMaxSeconds != nil {
		laneRotationMaxSeconds = *cfg.LaneRotationMaxSeconds
	}
	installationID, err := ensureInstallationID(stateDir)
	if err != nil {
		return windowsruntime.Profile{}, err
	}
