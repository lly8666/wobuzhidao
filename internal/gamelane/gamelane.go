...
var (
	...
	ErrWrongSession = errors.New("gamelane: wrong logical tunnel session")
...
func (e *Encoder) SessionID() SessionID {
	if e == nil {
		return SessionID{}
	}
	return e.sessionID
...
// WrapCopies assigns exactly one tunnel-wide PacketID and emits one
// lane-distinct plaintext envelope per authoritative logical lane. The caller
// sends each copy through that lane's independent LINK/FEC/record pipeline.
func (e *Encoder) WrapCopies(payload []byte, laneIDs []uint8) (packetID uint64, copies []LaneCopy, err error) {
	if e == nil || len(payload) == 0 || len(payload) > math.MaxUint16 || len(laneIDs) == 0 || len(laneIDs) > MaxLanes {
...
func NewDecoder(sessionID SessionID, replayWindow int) (*Decoder, error) {
	...
	return &Decoder{
		sessionID: sessionID,
		window:    uint64(replayWindow),
		seen:      make(map[uint64]struct{}, replayWindow),
	}, nil
}
...
func (d *Decoder) HighestPacketID() (uint64, bool) {
	if d == nil {
		return 0, false
	}
	return d.highest, d.haveHigh
}

func (d *Decoder) Recent() int {
	if d == nil {
		return 0
	}
	return len(d.seen)
...
// Add implements tunnel-wide first-arrival delivery across lanes. Unique
// PacketIDs may arrive out of order inside the bounded window; there is no
// expected PacketID and therefore no cross-lane HOL.
