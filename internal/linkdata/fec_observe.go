...
func (c *observedDecoderCodec) Reconstruct(shards [][]byte, present []bool) error {
	...
	if err == nil {
		c.mu.Lock(); c.success++; c.mu.Unlock()
	}
	return err
}
...
func observeFECWire(p *Path, h fec.BlockHeader) {
	...
	if int(h.ShardIndex) == fec.DataShards {
		n := int(h.DataCount)
		if n >= 1 && n <= fec.DataShards { s.hist[n]++ }
	}
}

func observeFECDecoder(p *Path, now time.Time) {
	v, ok := fecObserve.Load(p); if !ok || p.dec == nil { return }
	s := v.(*fecObserveState)
	d := p.dec.PressureStats()
	if d.InFlight > s.peakInFlight { s.peakInFlight = d.InFlight }
	if d.Retired > s.peakRetired { s.peakRetired = d.Retired }
	if d.RetiredIncomplete > s.peakRetiredIncomplete { s.peakRetiredIncomplete = d.RetiredIncomplete }
	if d.RetiredMissingSources > s.peakRetiredMissing { s.peakRetiredMissing = d.RetiredMissingSources }
	if d.Retired > s.lastRetired { s.retireEvents += uint64(d.Retired-s.lastRetired) }
...
func (p *Path) FECObserveStats() FECObserveStats {
	out := FECObserveStats{}
	if p == nil || p.dec == nil { return out }
	out.Decoder = p.dec.PressureStats()
	v, ok := fecObserve.Load(p); if !ok { return out }
	s := v.(*fecObserveState)
