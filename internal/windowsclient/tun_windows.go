...
func OpenTUN(name string) (*TUN, error) {
	...
	closeEvt, _, callErr := createEventW.Call(0, 1, 0, 0)
	if closeEvt == 0 {
		wintunEndSession.Call(session)
		wintunCloseAdapter.Call(adapter)
		return nil, wintunCallError("CreateEventW", callErr, nil)
	}
	return &TUN{adapter: adapter, session: session, readEvt: readEvt, closeEvt: closeEvt, name: name}, nil
}
...
func (t *TUN) endCall() { t.active.Done() }
...
// ReadPacket fails closed on non-IPv4 before copying bytes into the active
