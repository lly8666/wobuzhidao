...
func FuzzUnmarshal(f *testing.F) {
	seeds := [][]byte{{}, []byte("WBDC"), mustMarshal(Hello{MinProtocol:1,MaxProtocol:1}), mustMarshal(Accept{Protocol:1}), mustMarshal(Error{Code:ErrorPolicy,Message:"x"}), mustMarshal(Auth{Token:[]byte("token")}), mustMarshal(AuthOK{}), mustMarshal(Ping{Nonce:1}), mustMarshal(Pong{Nonce:1})}
