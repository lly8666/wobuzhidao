...
PHASES = [
    ...
    ("high30", 30_000_000_000, 90_000_000_000, 30),
...
def drop_score(seed, direction, phase, ordinal):
    ...
    phase_salt = {"high30": 43, "low5-recovery": 61, "post": 83}.get(phase, 0)
    if ordinal <= 0:
        return 0
...
def main():
    ...
    if scenario.get("phase_seconds") != [30, 60, 30] or scenario.get("drop_percent") != [5, 30, 5]:
        fail("30/60/30 or 5/30/5 scenario provenance")
    if scenario.get("fec_profile") != "off" or scenario.get("production_default_fec") != "off" or scenario.get("padding") != "off":
        fail("fixed FEC-off/padding-off policy")

    transitions = [x for x in network if x.get("event") == "phase_transition"]
    expected_transitions = [
        ("low5-initial", 0, 5),
        ("high30", 30_000_000_000, 30),
    ...
    if any(final.get(k, 0) != 0 for k in zero_fec_keys):
        fail("FEC-off inventory is nonzero")
    ...
    summary = {
        ...
        "scenario": "weaknet-300ms-120s-5-30-5",
    ...
    print(
        f"WBD_P5_WEAKNET_5305_PASS source_sha={args.source_sha} run={run_ordinal} seed={seed} "
        f"duration=120s delay=300ms drop=5-30-5 attempts={total_attempts} drops={total_drop} "
        ...
        f"fec=off repair={repair} padding=off wire_amp={wire_amp:.6f}"
    )
