...
def main():
    ...
    performance_errors = []
    def enforce(direction, stage, max_loss):
        row = gen[direction][stage]
        min_gp = (1.0 - max_loss / 100.0) * 0.99
    ...
    for direction in ("c2s", "s2c"):
        for stage in STAGES:
            enforce(direction, stage, desired[stage])

    probe = biz["stats"]
    probe_sent = probe.get("probe_sent", 0)
    probe_timeouts = probe.get("probe_timeouts", 0)
    probe_loss = 100.0 * probe_timeouts / probe_sent if probe_sent else None
    if args.scenario == "lossless" and probe_timeouts:
        performance_errors.append(
            f"lossless probe timeouts={probe_timeouts}/{probe_sent}"
        )

    late_after_10s = 0
    for receiver in (biz, target):
        wall = receiver["stats"].get("recv_wall_bytes_by_second", [])
        late_after_10s += sum(wall[130:]) if len(wall) > 130 else 0
    if late_after_10s:
        performance_errors.append(
            f"delivery persisted beyond 10s drain bytes={late_after_10s}"
        )
        ...
        for offset in range(0, 8):
            ...
            for sec in range(90 + offset, 93 + offset):
                for direction, sender, receiver in (
                    ...
                    gp = recv_b * 8 / 1e6
                    if gp < args.target_mbps * 0.95 * 0.99 or loss > 5.0:
                        ok = False
    ...
    result = {
        ...
        "drain": {"late_after_10s_bytes": late_after_10s},
        "rtt_baseline_gate": "DEFER_TO_ARTIFACT_ONLY_LOSS_TOLERANT_V1_AGGREGATE",
    }
    Path(args.output).write_text(json.dumps(result, indent=2, sort_keys=True) + "\n", encoding="utf-8")
